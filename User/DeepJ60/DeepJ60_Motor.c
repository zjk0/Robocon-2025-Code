...
void ReceiveCanDataToJ60MotorMotionData_new (ReceiveMotorDataInformation* ReceiveMotorData, uint8_t* ReceiveCanData) {
...
void ReceiveCanDataToJ60MotorData_new (MotorInformation* Motor, CanFrame* Can, uint8_t CommandID) {
    if (CommandID == CONTROL_MOTOR) {
        ReceiveCanDataToJ60MotorMotionData_new(&Motor->ReceiveMotorData, Can->ReceiveData);
    }
    else if (CommandID== GET_CONFIG) {
        ReceiveCanDataToJ60MotorConfig(&Motor->GetConfigReceiveData, Can->ReceiveData);
    }
    else if (CommandID == GET_STATUS_WORD) {
        ReceiveStatusWordToJ60MotorStatusWord(&Motor->MotorStatusWord, Can->ReceiveData);
    }
    else {
        ReceiveCommandStatusToJ60MotorCommandStatus(&Motor->MotorCommandStatus, Can->ReceiveData);
    }
}

void AnalyseJ60MotorReceiveData_new (MotorInformation* Motor) {
    ...
    ReceiveCanDataToJ60MotorData_new(&Motor[ReceiveMotorID - 1], &Can, ReceiveCommandID);
}
