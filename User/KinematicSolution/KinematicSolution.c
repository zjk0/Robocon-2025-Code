...
int IK_leg(float x_pos, float y_pos, float *angle_e, float *angle_i) {
    ...
    float angle_xOD, angle_COD, angle_xOC, beta; // 定义一些逆解需要的角
    ...
    beta = acos((l1 * l1 + loc2 - l2 * l2) / (2 * l1 * sqrt(loc2)));
    if (isnan(beta)) {
        return NAN_ERROR;
    }

    angle_link_e = beta - angle_xOC;
    angle_link_i = 3.14159 - angle_xOC - beta;
